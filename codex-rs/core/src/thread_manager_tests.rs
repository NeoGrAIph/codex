...
#[tokio::test]
async fn generic_exact_v1_resume_requires_persisted_pathless_subagent_source() {
    ...
    let caller_parent_id = ThreadId::new();
    let caller_source =
        SessionSource::SubAgent(codex_protocol::protocol::SubAgentSource::ThreadSpawn {
            parent_thread_id: caller_parent_id,
            depth: 1,
            agent_path: None,
            agent_nickname: None,
            agent_role: None,
        });
    let resumed_history = |persisted_source| {
        InitialHistory::Resumed(ResumedHistory {
            conversation_id: thread_id,
            history: Arc::new(vec![codex_protocol::protocol::RolloutItem::SessionMeta(
                SessionMetaLine {
                    meta: SessionMeta {
    ...
    let root_history = resumed_history(SessionSource::Exec);
    assert_eq!(
        normalize_multi_agent_runtime_intent(
            crate::agent::control::MultiAgentRuntimeIntent::Inherit,
            &root_history,
            Some(codex_protocol::protocol::ThreadHistoryMode::Legacy),
            &caller_source,
            Some(caller_parent_id),
            /*forked_from_thread_id*/ None,
            Some(&ThreadSource::Subagent),
            &config,
        )
        .expect("generic root resume should remain inherited"),
        crate::agent::control::MultiAgentRuntimeIntent::Inherit
    );

    let persisted_parent_id = ThreadId::new();
    let subagent_history = resumed_history(SessionSource::SubAgent(
        codex_protocol::protocol::SubAgentSource::ThreadSpawn {
            parent_thread_id: persisted_parent_id,
            depth: 1,
            agent_path: None,
            agent_nickname: None,
            agent_role: None,
    ...
    assert_eq!(
        ...
        .expect("persisted pathless V1 subagent should auto-select exact resume"),
        crate::agent::control::MultiAgentRuntimeIntent::ExactV1Resume
    );
}

#[tokio::test]
async fn exact_v1_resume_preserves_stopped_v2_runtime_mapping() {
    ...
    let occupant = manager
        ...
        .await
        .expect("V2 occupant should start");
    assert_eq!(
        occupant.thread.multi_agent_version(),
    ...
    occupant
        ...
        .shutdown_and_wait()
        .await
        .expect("V2 occupant should stop without leaving the manager");
    ...
    let initial_history = InitialHistory::Resumed(ResumedHistory {
        conversation_id: occupant.thread_id,
        history: Arc::new(vec![RolloutItem::SessionMeta(SessionMetaLine {
            meta: SessionMeta {
                session_id: occupant.thread_id.into(),
                id: occupant.thread_id,
    ...
    let result = manager
        ...
        .resume_thread_with_history_with_source(ResumeThreadWithHistoryOptions {
            ...
            environment_selections: Some(
                manager.default_environment_selections(&config.cwd, &config.workspace_roots),
            ),
            multi_agent_runtime: MultiAgentRuntimeIntent::ExactV1Resume,
        })
        .await;
    let Err(error) = result else {
        panic!("exact V1 resume must reject a stopped V2 occupant");
    };

    assert!(
        error
            .to_string()
            .contains("does not use the V1 multi-agent runtime")
    );
    let current = manager
        .get_thread(occupant.thread_id)
        .await
        .expect("stopped V2 occupant must remain mapped");
    assert!(Arc::ptr_eq(&current, &occupant.thread));
}
