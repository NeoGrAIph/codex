...
struct BlockingCloseAgentGraphStore {
    closed_write_started: Semaphore,
    release_closed_write: Semaphore,
    fail_closed_write: AtomicBool,
}

impl Default for BlockingCloseAgentGraphStore {
    fn default() -> Self {
        Self {
            closed_write_started: Semaphore::new(0),
            release_closed_write: Semaphore::new(0),
            fail_closed_write: AtomicBool::new(false),
        }
    }
}

impl codex_agent_graph_store::AgentGraphStore for BlockingCloseAgentGraphStore {
    ...
    fn set_thread_spawn_edge_status(
        ...
        Box::pin(async move {
            if status == codex_agent_graph_store::ThreadSpawnEdgeStatus::Closed {
                if self.fail_closed_write.load(Ordering::SeqCst) {
                    return Err(codex_agent_graph_store::AgentGraphStoreError::Internal {
                        message: "injected close write failure".to_string(),
                    });
                }
                self.closed_write_started.add_permits(1);
                self.release_closed_write
                    .acquire()
                    .await
                    .expect("close write release semaphore should remain open")
                    .forget();
            }
            Ok(())
        })
...
async fn projected_v1_cold_resume_restores_current_managed_authority() {
    const REMOVED_ROLE: &str = "removed-role";
    ...
    let target = session
        ...
        .spawn_agent_with_metadata(
            ...
            Some(SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
                ...
                agent_path: None,
                agent_nickname: None,
                agent_role: Some(REMOVED_ROLE.to_string()),
            })),
            SpawnAgentOptions {
    ...
    resume_config.config_layer_stack = resume_config
        .config_layer_stack
        .clone()
    ...
    assert_eq!(
        resumed
            ...
            .await
            .expect("resumed target should remain readable")
            .agent_role
            .as_deref(),
        Some(REMOVED_ROLE)
    );
    ...
    assert!(crate::exec_policy::child_uses_parent_exec_policy(
        &expected_exec_policy_parent_config,
        resumed_turn.config.as_ref(),
    ));
}
...
#[tokio::test]
async fn projected_v1_persisted_disabled_and_unresolved_targets_fail_before_effects() {
    let (mut session, mut turn) = make_session_and_context().await;
    let mut caller_config = (*turn.config).clone();
    caller_config
        .features
        .enable(Feature::MultiAgentV2)
    ...
    let state_db = init_state_db(&caller_config)
        .await
        .expect("sqlite state db should initialize");
    let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
        CodexAuth::from_api_key("dummy"),
        caller_config.model_provider.clone(),
        caller_config.codex_home.to_path_buf(),
        Arc::new(codex_exec_server::EnvironmentManager::default_for_tests()),
        Some(state_db),
    );

    let mut disabled_config = caller_config.clone();
    disabled_config
        .features
        .disable(Feature::MultiAgentV2)
        .expect("test config should allow V2 disable");
    disabled_config.agents_enabled = false;
    let disabled = manager
        .start_thread(disabled_config)
        .await
        .expect("Disabled target should start");
    assert_eq!(
        disabled.thread.multi_agent_version(),
        Some(codex_protocol::protocol::MultiAgentVersion::Disabled)
    );
    disabled
        .thread
        .inject_user_message_without_turn("persist Disabled target".to_string())
        .await;
    disabled.thread.ensure_rollout_materialized().await;
    disabled
        .thread
        .flush_rollout()
        .await
        .expect("Disabled target rollout should flush");
    let disabled_id = disabled.thread_id;
    manager
        .agent_control()
        .shutdown_live_agent(disabled_id)
        .await
        .expect("Disabled target should unload");

    let unresolved_environments =
        manager.default_environment_selections(&caller_config.cwd, &caller_config.workspace_roots);
    let unresolved = manager
        .start_thread_with_options(crate::thread_manager::StartThreadOptions {
            config: caller_config.clone(),
            ...
            parent_trace: None,
            environments: unresolved_environments,
            thread_extension_init: codex_extension_api::ExtensionDataInit::default(),
            supports_openai_form_elicitation: false,
        })
        .await
        .expect("Paginated target should start");
    unresolved
        .thread
        .inject_user_message_without_turn("persist unresolved target".to_string())
        .await;
    unresolved.thread.ensure_rollout_materialized().await;
    unresolved
        .thread
        .flush_rollout()
        .await
        .expect("Paginated target rollout should flush");
    let unresolved_id = unresolved.thread_id;
    let unresolved_rollout_path = unresolved
        .thread
        .rollout_path()
        .expect("Paginated target should have a rollout");
    manager
        .agent_control()
        .shutdown_live_agent(unresolved_id)
        .await
        .expect("Paginated target should unload");

    let mut unresolved_lines = std::fs::read_to_string(&unresolved_rollout_path)
        .expect("unresolved target rollout should be readable")
        .lines()
        .map(|line| serde_json::from_str::<RolloutLine>(line).expect("parse rollout line"))
        .collect::<Vec<_>>();
    let RolloutItem::SessionMeta(meta_line) = &mut unresolved_lines[0].item else {
        panic!("unresolved target rollout should start with session metadata");
    };
    meta_line.meta.multi_agent_version = None;
    let unresolved_rollout = unresolved_lines
        .iter()
        .map(|line| serde_json::to_string(line).expect("serialize rollout line"))
        .collect::<Vec<_>>()
        .join("\n");
    std::fs::write(&unresolved_rollout_path, format!("{unresolved_rollout}\n"))
        .expect("unresolved runtime marker should persist");

    session.services.agent_control = manager.agent_control();
    set_turn_config(&mut turn, caller_config);
    let session = Arc::new(session);
    let turn = Arc::new(turn);
    for (target_id, expected_error_fragment) in [
        ...
        let Err(error) = ProjectedCloseAgentHandler
            ...
            panic!("projected close_agent should reject target {target_id}");
        };
        let FunctionCallError::RespondToModel(message) = error else {
            panic!("expected a model-facing target-version error");
        };
...
#[tokio::test]
async fn projected_v1_live_close_persists_closed_before_shutdown() {
    let (_session, turn) = make_session_and_context().await;
    let mut config = (*turn.config).clone();
    config
        .features
    ...
    .await
    .expect("close should attempt the durable status update")
    .expect("close write semaphore should remain open")
    .forget();
    assert!(
        manager.get_thread(target_id).await.is_ok(),
        "the target must remain live until the Closed graph write finishes"
    );
    ...
    graph_store.fail_closed_write.store(true, Ordering::SeqCst);
    let target_with_failing_store = manager
        .agent_control()
        .spawn_agent_with_metadata(
            config.clone(),
            vec![UserInput::Text {
                text: "close despite graph write failure".to_string(),
                text_elements: Vec::new(),
            }],
            Some(SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
                parent_thread_id: root.thread_id,
        ...
        .await
        .expect("second exact V1 target should start");
...
#[tokio::test]
async fn projected_v1_cross_root_uuid_addressing_remains_allowed() {
    let (mut session, mut turn) = make_session_and_context().await;
    let mut v2_config = (*turn.config).clone();
    v2_config
        .features
        ...
        .expect("test config should allow feature update");
    let manager = thread_manager();
    let caller_root = manager
        .start_thread(v2_config.clone())
        .await
        .expect("caller V2 root should start");
    let owner_root = manager
        .start_thread(v2_config.clone())
        .await
        .expect("owner V2 root should start");
    let target = manager
        .agent_control()
        .spawn_agent_with_metadata(
            v2_config.clone(),
            vec![UserInput::Text {
                text: "persist cross-root V1 target".to_string(),
                text_elements: Vec::new(),
            }],
            Some(SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
        ...
        .await
        .expect("owner root should spawn exact V1 target");
    let target_id = target.thread_id;
    let target_thread = manager
        .get_thread(target_id)
        .await
        .expect("cross-root V1 target should be live");
    target_thread.ensure_rollout_materialized().await;
    target_thread
        .flush_rollout()
        .await
        .expect("cross-root V1 target should persist");

    session.services.agent_control = manager.agent_control();
    session.thread_id = caller_root.thread_id;
    set_turn_config(&mut turn, v2_config);
    let session = Arc::new(session);
    let turn = Arc::new(turn);

    ProjectedSendInputHandler
        .handle(invocation(
            Arc::clone(&session),
            Arc::clone(&turn),
            "send_input",
            function_payload(json!({
                ...
                "message": "cross-root follow-up"
            })),
        ))
        .await
        .expect("projected V1 send should accept a foreign-root UUID");
    assert!(
        manager
            .captured_ops()
            .iter()
            .any(|(thread_id, op)| *thread_id == target_id && matches!(op, Op::UserInput { .. }))
    );
    ...
    manager
        ...
        .shutdown_live_agent(target_id)
        .await
        .expect("cross-root V1 target should unload");
    assert!(manager.get_thread(target_id).await.is_err());
    ...
    ProjectedResumeAgentHandler
        ...
        .await
        .expect("projected V1 resume should accept a persisted foreign-root UUID");
    assert_eq!(
        manager
            .get_thread(target_id)
            .await
            .expect("foreign-root V1 target should resume")
            .multi_agent_version(),
        Some(codex_protocol::protocol::MultiAgentVersion::V1)
    );
    ...
    ProjectedCloseAgentHandler
        ...
        .await
        .expect("projected V1 close should accept a foreign-root UUID");
    assert!(manager.get_thread(target_id).await.is_err());

    manager
        .agent_control()
        .shutdown_live_agent(caller_root.thread_id)
        .await
        .expect("caller root cleanup should succeed");
    manager
        .agent_control()
