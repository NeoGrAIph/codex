...
impl ToolHandler for Handler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        let thread_note = normalize_thread_note(args.note.as_deref());
        session
            .services
            .agent_control
            .set_thread_note(session.conversation_id, thread_note.clone())
            .await
            .map_err(|err| collab_agent_error(session.conversation_id, err))?;

        Ok(SetThreadNoteResult { thread_note })
    }
}
