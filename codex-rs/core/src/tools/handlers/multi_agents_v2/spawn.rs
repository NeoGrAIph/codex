...
impl ToolHandler for Handler {
    ...
    fn handle(
        ...
        Box::pin(async move {
            ...
            let spawn_cwd = resolve_spawn_agent_cwd(&turn.cwd, args.cwd.as_deref())?;
            if spawn_cwd.is_explicit() && fork_mode.is_some() {
                return Err(FunctionCallError::RespondToModel(
                "cwd cannot be combined with forked history in this release; set fork_turns to \"none\" when using cwd.".to_string(),
            ));
            }
            let role_name = args
                .agent_type
            ...
            let mut config = match &spawn_cwd {
                SpawnAgentCwd::Inherited => {
                    build_agent_spawn_config(&base_instructions, turn.as_ref())?
                }
                SpawnAgentCwd::Explicit(_) => {
                    build_agent_spawn_config_for_cwd(
                        &base_instructions,
                        turn.as_ref(),
                        spawn_cwd.clone(),
                    )
            ...
            let environments = if spawn_cwd.is_explicit() {
                ...
                Some(turn.environments.to_selections())
            };
