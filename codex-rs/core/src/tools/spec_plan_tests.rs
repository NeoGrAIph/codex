...
#[derive(Debug)]
struct NoNamespaceToolsProvider {
    delegate: Arc<dyn ModelProvider>,
}

impl ModelProvider for NoNamespaceToolsProvider {
    ...
    fn auth(&self) -> codex_model_provider::ModelProviderFuture<'_, Option<CodexAuth>> {
        let delegate = Arc::clone(&self.delegate);
        Box::pin(async move { delegate.auth().await })
    }

    fn account_state(&self) -> codex_model_provider::ProviderAccountResult {
...
#[tokio::test]
async fn projected_v1_is_hidden_without_namespace_tool_support() {
    let plan = probe(|turn| {
        set_feature(turn, Feature::MultiAgentV2, /*enabled*/ true);
        turn.provider = Arc::new(NoNamespaceToolsProvider {
            delegate: Arc::clone(&turn.provider),
        });
    })
    .await;
...
#[tokio::test]
async fn projected_v1_is_hidden_for_guardian_reviewer() {
    let plan = probe(|turn| {
        set_feature(turn, Feature::MultiAgentV2, /*enabled*/ true);
