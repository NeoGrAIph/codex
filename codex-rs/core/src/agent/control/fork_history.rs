...
fn sanitize_response_item(
    ...
    content.retain(|content_item| !is_multi_agent_mode_fragment(content_item));
    !content.is_empty()
}
...
fn is_multi_agent_mode_fragment(content_item: &ContentItem) -> bool {
    let ContentItem::InputText { text } = content_item else {
        return false;
    };
    let text = text.trim_start();
    text.get(..MULTI_AGENT_MODE_OPEN_TAG.len())
        .is_some_and(|prefix| prefix.eq_ignore_ascii_case(MULTI_AGENT_MODE_OPEN_TAG))
}
