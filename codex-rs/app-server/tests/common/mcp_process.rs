...
impl McpProcess {
    ...
    /// Send a `thread/note/set` JSON-RPC request.
    pub async fn send_thread_set_note_request(
        &mut self,
        params: ThreadSetNoteParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
        self.send_request("thread/note/set", params).await
    }
