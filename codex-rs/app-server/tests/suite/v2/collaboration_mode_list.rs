...
//! includes the plan, interactive, and default modes with their default model and reasoning effort
...
async fn list_collaboration_modes_returns_presets() -> Result<()> {
    ...
    let expected = vec![plan_preset(), interactive_preset(), default_preset()];
...
/// Builds the interactive preset that the list response is expected to return.
fn interactive_preset() -> CollaborationModeMask {
    let presets = builtin_collaboration_mode_presets();
    presets
        .into_iter()
        .find(|p| p.mode == Some(ModeKind::Interactive))
        .unwrap()
}
