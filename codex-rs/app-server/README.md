...
- `thread/metadata/update` — patch stored thread metadata without resuming it; currently supports persisted `gitInfo` fields and metadata-only `threadNote`. Setting or clearing `threadNote` emits `thread/note/updated` after the response.
...
- `agent/message/send` — experimental; queue an encrypted message from one loaded thread/agent to a path-backed descendant agent without starting a turn. Server-side ownership checks reject root/self/sibling/pathless/closed/error targets.
- `agent/followup/send` — experimental; send an encrypted follow-up from one loaded thread/agent to an idle path-backed descendant and trigger a turn using the native multi-agent runtime. Server-side ownership checks mirror `agent/message/send`.
- `agent/close` — experimental; close a path-backed spawned descendant agent through native `AgentControl::close_agent`; returns the previous `CollabAgentState`.
- `agent/dismiss` — experimental; hide a path-backed spawned descendant from Agent Window through native thread visibility metadata without closing, archiving, deleting or unloading the agent; returns `{}`.
- `agent/retry` — experimental; spawn a fresh sibling for an eligible path-backed descendant from its durable initial task; returns the new `threadId`.
- `agent/stopAll` — experimental; close live descendants in the author thread's root/subtree scope through native `AgentControl::close_agent`; returns affected/stopped counts and per-target failures.
...
- `model/list` — list available models (set `includeHidden: true` to include entries with `hidden: true` and experimental `includeConfiguredProviders: true` to include configured non-active providers that are enabled in the picker), with model-advertised string reasoning effort options in the catalog's intended progression order, provider identity, `additionalSpeedTiers`, `serviceTiers`, optional `defaultServiceTier`, optional legacy `upgrade` model ids, optional `upgradeInfo` metadata (`model`, `upgradeCopy`, `modelLink`, `migrationMarkdown`), and optional `availabilityNux` metadata. Clients should preserve the `supportedReasoningEfforts` array order rather than deriving order from the effort names.
- `modelProvider/capabilities/read` — read provider-level capabilities for the currently configured model provider.
- `modelProvider/list` — experimental; list configured model providers with picker visibility, auth status and manageable state. OpenAI is native and cannot be disabled through this surface.
- `modelProvider/config/write` — experimental; enable/disable a non-native provider in the picker or make it active. When making a provider active, app-server also writes that provider's default model.
- `modelProvider/auth/write` — experimental; store a managed API key for a non-native provider in the local secrets backend and refresh loaded thread runtime config.
- `modelProvider/auth/remove` — experimental; remove the managed API key for a non-native provider and refresh loaded thread runtime config.
- `agentRole/toolSelectionCatalog/read` — experimental; read a loaded thread's native runtime tool-selection catalog and unmatched role allowlist entries for role-authoring UI. This is a read-only projection, not a role management/write API.
- `agentRole/toolSelection/set` — experimental; replace `[tool_selection]` for a discovered user role file under `$CODEX_HOME/agents/*.toml`, validate through the native role parser, and refresh loaded thread config. Built-in roles and external `config_file` roles are rejected.
- `agentRole/actionPolicy/set` — experimental; replace `[subagent_action_policy]` for a discovered user role file under `$CODEX_HOME/agents/*.toml`, validate through the native role parser, and refresh loaded thread config. Built-in roles and external `config_file` roles are rejected.
