...
- `skills/remote/read` — list public remote skills (FORK: disabled by policy; returns a JSON-RPC error by default).
- `skills/remote/write` — download a public remote skill by `hazelnutId` (FORK: disabled by policy; returns a JSON-RPC error by default).
