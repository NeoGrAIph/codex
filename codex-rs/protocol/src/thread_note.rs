...
mod tests {
    ...
    fn normalize_thread_note_preserves_structured_form_and_spacing() {
        assert_eq!(
            normalize_thread_note(Some(
                "  Назначение: Исследователь репозитория| Компетенции: docs/features; AGENTS.md  "
            )),
            Some(
                "Назначение: Исследователь репозитория | Компетенции: docs/features; AGENTS.md"
                    .to_string()
    ...
    fn normalize_thread_note_requires_non_empty_purpose() {
        assert_eq!(
            normalize_thread_note(Some("Назначение:   | Компетенции: docs/features")),
            None
        );
    }
    ...
    fn parse_thread_note_supports_legacy_and_structured_values() {
        ...
        assert_eq!(
            parse_thread_note("Назначение: Исследователь | Компетенции: docs/features; AGENTS.md"),
            Some(StructuredThreadNote {
                purpose: "Исследователь".to_string(),
                competencies: "docs/features; AGENTS.md".to_string(),
            })
        );
    }

    #[test]
    fn escape_xml_text_escapes_xml_significant_chars() {
        assert_eq!(
            escape_xml_text("A & B <tag>"),
            "A &amp; B &lt;tag&gt;".to_string()
        );
    }
}
