...
function loadExpectedHostName() {
  ...
  throw new Error(
    `Could not read extensionHostName from ${resolveSiblingScriptPath(
      CHROME_EXTENSION_ID_CONFIG_FILENAME,
    )}.`,
  );
}
...
function getNativeHostManifestStatus() {
  ...
  const hostPathExists = hostPath != null && fs.existsSync(hostPath);
...
function describeManifestProblem({
  ...
  if (!hostPathExists) problems.push("manifest path is missing or does not exist");
...
function main() {
  ...
  if (json) console.log(JSON.stringify(result, null, 2));
    ...
    if (result.hostPath) console.log(`Host binary path: ${result.hostPath}`);
    if (result.hostPathExists != null) {
      console.log(`Host binary exists: ${result.hostPathExists ? "yes" : "no"}`);
    }
