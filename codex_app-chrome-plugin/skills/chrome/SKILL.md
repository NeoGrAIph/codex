...
Chrome is the routing touchpoint for the Codex App Chrome Extension:
...
- If communication with the Codex App Chrome Extension ultimately fails, even after checks, do not attempt to complete the user's request using applescript, bash commands or any other scripting methods.
- Do not install or repair the native host yourself. If native host setup appears broken, tell the user to reinstall the Codex App Chrome plugin from the Codex App plugin UI.
...
### 4. The Codex App Chrome Extension is not installed

Keep the first response short and non-technical unless the user asks for more information.

If the Codex App Chrome Extension is missing, tell the user:

`Cannot communicate with the Codex App Chrome Extension. Confirm that the extension is installed and enabled in Chrome.`

Ask the User if you can open the Google Chrome Extension Manager so they can verify that the unpacked extension is installed. ALWAYS wait for a user response before taking action. ALWAYS refer to the extension as the [Codex App Chrome Extension](chrome://extensions/), and not by its extension ID.

The expected unpacked extension ID is configured in `scripts/extension-id.json`.


### 4. The Codex App Chrome Extension is not enabled

Keep the first response short and non-technical unless the user asks for more information.

If the Codex App Chrome Extension is not enabled ask the User if you can open the Google Chrome Extension Manager so they can verify that the extension is enabled. ALWAYS wait for a user response before taking action. Always refer to the Google Chrome Extension Manager as [Google Chrome Extension Manager](chrome://extensions/).


### 5. Codex App Chrome Extension is installed and enabled, the manifest file is installed, but communication still fails
...
If the issue is specifically the native host or extension-backed install path, or if communication still fails after opening a Chrome window and retrying setup once, tell the user to reinstall the Codex App Chrome plugin from the Codex App plugin UI. Never import or run `scripts/installManifest.mjs` yourself.
...
`To enable file upload, go to chrome://extensions in Chrome, click Details under the Codex App Chrome extension, and enable "Allow access to file URLs."`
...
### check-fork-static.js

This script verifies that the unpacked extension and plugin agree on fork identity before any browser or native host runtime checks are attempted. It validates the manifest-derived extension ID, native host name, key visible rebrand strings, required background method names, and absence of the official extension ID/native host wiring in fork-owned files.

From the plugin root, use:

```
scripts/check-fork-static.js
```
...
The default extension root is the sibling `codex-chrome-extension-unpacked` directory. For debugging or tests, override it with `CODEX_CHROME_EXTENSION_ROOT=/path/to/unpacked-extension`.

### check-native-host-manifest.js

This script checks whether the Chrome Native Messaging Host manifest exists for the configured native host name and allows the Chrome extension ID from `scripts/extension-id.json`. On Windows it also checks the Chrome NativeMessagingHosts registry key. On Linux, native host installation requires either a bundled `extension-host/linux/<arch>/extension-host` binary or `CODEX_CHROME_EXTENSION_HOST_PATH=/path/to/extension-host` during plugin installation. It exits `0` when correct, `1` when missing or incorrect, and `2` for usage or runtime errors.
...
This module is normally called by the plugin installation flow. For local fork smoke tests, it can also be inspected without writing files:

```
node scripts/installManifest.mjs --dry-run --json
```

On Linux, set `CODEX_CHROME_EXTENSION_HOST_PATH=/path/to/extension-host` when the fork does not bundle `extension-host/linux/<arch>/extension-host`.
