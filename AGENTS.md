...
3. Worker не делает commit, push, merge или cherry-pick. Результат worker-а: итоговый diff/patch, summary, checks, evidence и known risks.
...
6. После зелёного аудита main/root thread применяет итоговый worker diff в основной checkout как обычные незакоммиченные изменения. Worker commits не являются source-of-truth и не переносятся через merge/cherry-pick.
