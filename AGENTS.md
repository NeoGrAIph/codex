...
- Wrap each introduced fork block with paired markers:
  ...
  - `FORK COMMIT CLOSE: ...`
- For a single-line fork customization, use:
  - `FORK COMMIT [<FEATURE>]: ...`
- Feature name inside `[...]` must be the short feature code in the original case (case-sensitive).
...
- For new files added for a fork feature, add a header comment:
  - `// FORK COMMIT NEW FILE [<FEATURE>]: ...`
  - `// Role: ...`

Example for this repository's current feature (SubAgentsWindow / SAW):

- `FORK COMMIT OPEN [SAW]: ...`
- `FORK COMMIT CLOSE: ...`
...
- **Implementation map**
  ...
  - where fork markers (`FORK COMMIT OPEN [<FEATURE>]` / `FORK COMMIT CLOSE`) are expected.
- **Verification matrix**
