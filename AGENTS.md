...
- Use `scripts/codex-fork-build.sh` from the repository root to build the fork release binary at `codex-rs/target/release/codex`, remove debug and unneeded symbol tables with `strip`, and record the build hash. Local fork builds do not preserve a debug-symbol sidecar or archive.
- Use `scripts/2_codex-fork-install-binary.sh` after a successful build to актуализировать the executable `codex` binary resolved from `PATH` plus the managed app-server runtime targets with the binary built from this fork. The script rejects an unstripped source that still contains `.debug_info` or `.symtab`. It актуализирует the managed standalone runtime plus `~/.nvm/versions/node/v22.22.0/bin/codex` and `~/.nvm/versions/node/v22.22.0/lib/node_modules/@openai/codex/node_modules/@openai/codex-linux-x64/vendor/x86_64-unknown-linux-musl/bin/codex`; it does not stop, kill, start, or restart existing app-server processes.
- Use `scripts/2_codex-fork-install-binary.sh --dry-run` to verify source and target paths without changing files.
- Use `codex-fork restore-upstream-runtime` only as an explicit rollback/test action when local app-server runtime targets must be restored to the upstream npm package; pass `--dry-run` to preview and `--skip-npm-install` to reuse the currently installed npm package. This delegates to `scripts/3_codex-fork-restore-upstream-runtime.sh`, stops existing `codex app-server` processes, runs `npm install -g @openai/codex` unless skipped, copies the upstream npm vendor binary into the managed standalone runtime, the nvm `codex` entrypoint and the npm vendor binary, leaves `codex-rs/target/release/codex` unchanged, and does not explicitly start or restart the daemon.
