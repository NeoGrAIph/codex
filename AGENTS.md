...
- Use `scripts/codex-fork-sync-runtime.sh` after a successful build when the Codex app managed app-server runtime should pick up the fork binary. This script stops existing app-server processes, including targeted socket-owner termination for unmanaged daemons and `pkill -f 'rg codex app-server|/codex app-server'` for current app-server processes, then актуализирует the managed standalone runtime plus `~/.nvm/versions/node/v22.22.0/bin/codex` and `~/.nvm/versions/node/v22.22.0/lib/node_modules/@openai/codex/node_modules/@openai/codex-linux-x64/vendor/x86_64-unknown-linux-musl/bin/codex`; it must not explicitly start or restart the daemon because the next client request should start it through the normal app-server path.
