...
if [ -z "$REPO" ]; then
  echo "Failed to determine repository root from scripts/codex-fork-restore-upstream-runtime.sh" >&2
  exit 1
fi
...
FORK_RELEASE_CODEX="$REPO/codex-rs/target/release/codex"
...
runtime_targets() {
  printf '%s\n' "$MANAGED_CODEX" "$NPM_CODEX" "$VENDOR_CODEX" "$FORK_RELEASE_CODEX"
}
