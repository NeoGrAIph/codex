...
| [Namespace `multi_agent_v1` рядом с V2](tool-multi-agent-v1-add-to-v2.md) | Legacy V1 UUID-based tools рядом с нативным V2 namespace, но с общей canonical V2 identity, completion, authorization и resume semantics. Польза: модель может явно выбрать V1 или V2 surface без отключения второго и без pathless child runtime. | Draft/uncommitted production hardening на `fork/144.1`; verification in progress 2026-07-10 | Donor activation: `fork/142`, `dcf47dfce3`; временный source of truth: полный workspace diff `fork/144.1` |
...
Статус каждой staged/uncommitted возможности фиксируется в её строке и feature docs относительно указанной там ветки. До отдельного commit-запроса временным source of truth для такой возможности остаётся полный workspace diff соответствующего checkout, включая tracked и относящиеся к feature untracked файлы; состояние index само по себе не определяет границы candidate.
