...
| [Provider-aware models and DeepSeek](provider-aware-models-deepseek.md) | Поддержка моделей разных providers, включая DeepSeek через Chat Completions. Польза: пользователь/agent выбирает не только model, но и корректный provider. | Draft-перенос на `feature/140/provider-model-catalog`; исторически реализовано в `fork/130`/`chrome_plugin`; native `rust-v0.140.0` покрывает частично | `feature/140/provider-model-catalog`, `fork/130`, `chrome_plugin` |
