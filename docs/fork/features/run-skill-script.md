...
- Hooks: `RunSkillScriptHandler` publishes Bash PreToolUse/PostToolUse payloads from the outer registry dispatch, supports PreToolUse command rewrite through an internal rewritten-command field plus handler-generated token, rejects model-supplied rewrite fields without a valid token, and delegates exactly one process execution to `ExecCommandHandler`.
...
- 2026-06-18: audit hardening: hook command rewrites now require an internal token generated by `RunSkillScriptHandler`; model-supplied `_hook_updated_cmd` is rejected before unified exec delegation.
