...
Status: `partial`. Native release имеет defaults/config for `DEFAULT_AGENT_MAX_THREADS = Some(6)`, `DEFAULT_AGENT_MAX_DEPTH = 2`, `DEFAULT_MULTI_AGENT_V2_MAX_CONCURRENT_THREADS_PER_SESSION = 4`, wait min/default/max `10_000/30_000/3_600_000`, `MultiAgentV2Config`, v2 wait options and tests. Не хватает fork exact tuning: legacy `agents.max_threads` conflicts with v2 and v2 default concurrency is 4 threads/session including root rather than fork-style 12 agents.
