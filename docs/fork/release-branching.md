...
Этот документ является обязательной инструкцией для создания новой рабочей ветки форка на основании upstream stable-релиза Codex. Не используй последнюю локально созданную ветку как источник истины: новая baseline-ветка всегда создаётся от upstream release tag или от dereferenced release commit, а рабочая fork-ветка создаётся уже от этой локальной baseline-ветки.

## Цель

- создать неизменяемую локальную baseline-ветку для upstream-релиза;
- создать рабочую ветку форка от локальной baseline-ветки, а не напрямую от release commit;
...
git switch -c "fork/${release_line}" "fork/${release_line}-upstream"
git branch --set-upstream-to "fork/${release_line}-upstream" "fork/${release_line}"
```

Не создавай рабочую ветку форка напрямую от `$release_commit` и не используй `git switch -C` для обычного создания новой release-ветки: `fork/<release-line>` должен быть создан именно от локального ref `fork/<release-line>-upstream`, чтобы workflow отражал source-of-truth baseline.
...
Если рабочая ветка `fork/<release-line>` уже существует и была создана неверно, не исправляй её через `git switch -C`. Удали или переименуй её явным действием после проверки статуса, затем создай заново от `fork/<release-line>-upstream`.
...
- рабочая ветка была создана от локальной baseline-ветки `fork/<release-line>-upstream`, а не напрямую от `$release_commit`;
...
- Создавай baseline-ветку `fork/<release-line>-upstream` только от upstream release tag или dereferenced release commit.
- Создавай рабочую ветку `fork/<release-line>` только от локальной baseline-ветки `fork/<release-line>-upstream`.
