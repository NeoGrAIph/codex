...
- Для allowlist используется внутреннее имя `web_search`; в UI/ответах инструмент может отображаться как `web.run` (алиас).
- При `shutdown` родителя каскадно закрываются только `ThreadSpawn`-потомки (spawn_agent).
...
- **Политика инструментов** — `spawn_agent` использует базовую allow/deny политику
  (глобальный policy), не наследуя runtime allowlist родителя-оркестратора.
