...
Focused verification should cover `codex-core` because both tools are planned and dispatched there, and `codex-mcp` because manager metadata APIs changed. No app-server schema regeneration is expected because this iteration adds no protocol type or app-server command.
