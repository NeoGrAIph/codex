...
- Scope исследования: stable releases `rust-v0.86.0`-`rust-v0.145.0`.
- Start anchor: `rust-v0.86.0` -> `18057fdc30b10168ab18e8c1bb86c7a0a7772191`, `2026-01-15T16:46:06-08:00`.
- Current anchor: `rust-v0.145.0` -> `25af12f7e61572b0bc18ddb1008be543b91519b0`, `2026-07-21T10:29:35-07:00`.
- Boundary `rust-v0.144.4^{}` -> `rust-v0.145.0^{}`: left/right `7/342`, merge-base `3380969a29134630d56feb6218e8e8dcc5e8196d`. Patch-line commits имеют upstream equivalents/reworks (`dc23c7bcc8`, `22781d4001`, `8cf9a1b1f8`, `ea0fd84d94`, `769a5de257`, `60b9b551c1`), поэтому right-only inventory классифицируется по содержимому, а не по старым SHA.
...
- Финальная форма на `rust-v0.144.4` проверена командой `git grep -n -E 'ThreadItemsList|ancestor_thread_id|history_mode|last_turn_id|allow_provider_model_fallback|flush_transcript_tail' rust-v0.144.4^{} -- codex-rs/app-server-protocol codex-rs/app-server codex-rs/thread-store codex-rs/protocol` и просмотром registry в `codex-rs/app-server-protocol/src/protocol/common.rs`.
...
### `EVID-144.2-no-change`

- Диапазон: `rust-v0.144.1^{}..rust-v0.144.2^{}`; release anchor: `a6645b6b8a656360fa16fb7e1c6721d0697d3d6a`, `2026-07-12T20:24:45-07:00`; размер right-only множества: 2 commits; topology left/right `1/2`, merge-base `77c42a202aa7e89189c1126021dcc054b1795e1d`.
- Включены: нет.
- Результат: `32649bc5e6591ad8ea0b7b8ce073df447565ec7c` откатывает Guardian policy/prompt/tests и core tool planning, но не меняет app-server `thread/*`, ThreadStore, state, rollout или protocol thread surfaces.

### `EVID-144.3-resume-model-settings`

- Диапазон: `rust-v0.144.2^{}..rust-v0.144.3^{}`; release anchor: `78ad6e6bfd1d3b6a209acd3ef82172a96b25179c`, `2026-07-12T22:21:53-07:00`; размер right-only множества: 2 commits; topology left/right `1/2`, merge-base `32649bc5e6591ad8ea0b7b8ce073df447565ec7c`.
- Включён: `8a4d35a1e100efc2c64b72515668a84da663f067`.
- Подтверждение: diff затрагивает app-server `thread/resume`, state extraction, local/in-memory ThreadStore metadata и TUI app-server resume client. `EventMsg::ThreadSettingsApplied` обновляет persisted и live model/provider/reasoning/cwd/approval/permissions metadata; clearable `ThreadMetadataPatch.reasoning_effort` сохраняет explicit unset; cold resume восстанавливает metadata только при отсутствии явного model override.
- Wire disposition: app-server protocol types, registry, generated schemas и README не изменены; это lifecycle/persistence correction поверх существующих `thread/settings/update`, `thread/settings/updated` и `thread/resume`.
- Override semantics: explicit request/config overrides имеют приоритет; сохранённое отсутствие reasoning effort не заменяется current config default; TUI не должен передавать implicit current model/provider/effort или default service tier как resume override.
- Tests: `thread_resume_preserves_acknowledged_model_effort_and_approvals_reviewer_update`, `thread_settings_applied_updates_resume_metadata`, `thread_settings_applied_updates_live_metadata`, `update_thread_metadata_updates_permission_profile_and_reasoning_effort`, `thread_metadata_patch_round_trips_optional_clears`, `thread_resume_params_can_restore_persisted_model_settings`, `persisted_resume_does_not_forward_implicit_service_tier`, `resume_model_settings_preserves_only_explicit_model_overrides`.
...
### `EVID-144.4-no-change`

- Диапазон: `rust-v0.144.3^{}..rust-v0.144.4^{}`; release anchor: `8c68d4c87dc54d38861f5114e920c3de2efa5876`, `2026-07-13T21:20:37-07:00`; размер right-only множества: 2 commits; topology left/right `1/2`, merge-base `8a4d35a1e100efc2c64b72515668a84da663f067`.
- Включены: нет.
- Результат: `d82b7e5d4c1c274bee0eb55f92ec12d017e78634` добавляет `ModelMessages.auto_review.policy` и применяет его в Guardian session configuration. Изменение `codex-rs/protocol/src/openai_models.rs` является model-catalog near-miss и не меняет app-server thread protocol, ThreadStore или persistence.

### Команды прохода `rust-v0.142.5` -> `rust-v0.144.4`

```bash
git rev-parse 'rust-v0.142.5^{}' 'rust-v0.143.0^{}' 'rust-v0.144.0^{}' 'rust-v0.144.1^{}' 'rust-v0.144.2^{}' 'rust-v0.144.3^{}' 'rust-v0.144.4^{}'
...
git rev-list --count 'rust-v0.144.1^{}..rust-v0.144.2^{}'
git rev-list --count 'rust-v0.144.2^{}..rust-v0.144.3^{}'
git rev-list --count 'rust-v0.144.3^{}..rust-v0.144.4^{}'
git log --format='%H%x09%aI%x09%s' 'rust-v0.142.5^{}..rust-v0.144.4^{}' -- codex-rs/app-server codex-rs/app-server-protocol codex-rs/thread-store codex-rs/core/src/thread_manager.rs codex-rs/core/src/session codex-rs/rollout codex-rs/protocol/src
git log --format='%H%x09%aI%x09%s' 'rust-v0.142.5^{}..rust-v0.144.4^{}' | rg -i 'thread|session|resume|rollout|history|realtime|websocket|app.server|archive|fork|turn'
git show -s --format='%H%n%cI%n%s%n%B' rust-v0.143.0^{} rust-v0.144.0^{} rust-v0.144.1^{} rust-v0.144.2^{} rust-v0.144.3^{} rust-v0.144.4^{}
...
- `EVID-145-paginated-storage`: `414217dc8a`, `bfe31598c7`, `5c19155cbd`, `0ef9fa4d65`, `b24aa20107`; JSONL ordinals, dedicated SQLite storage и materialization/checkpoints.
- `EVID-145-paginated-app-server`: `e0a3641e6c`, `da61f7d8e1`, `0fb559f0f6`, `c1862b8db4`; paginated start/resume/read/list, turns/items pages и occurrence search.
- `EVID-145-paginated-lineage-metadata`: `69acc71cb1`, `2be648ba4a`, `5a208c1fc3`, `86102db5a1`, `19b2273d8a`, `2793c826e8`, `b7e39aa316`; inherited prefixes, lineage, names, Git/memory metadata и fail-closed rejection неизвестного history mode.
- `EVID-145-fork-resume-context`: `d88db19144`, `592467fb96`, `769a5de257`, `088239294a`; сохранение thread context при retry/edit, bounded rollout suffix для model context, explicit advanced-reasoning selection и восстановление V2 agent identities при root-thread resume.
- `EVID-145-thread-realtime-history`: `2e1607ee2f`, `b03545b5b8`, `025db22058`, `312caf176a`, `6f785632b0`; Frameless Bidi, streamed/channel handoffs, realtime V3 `initialItems` и audio preservation.
...
git log --no-merges --reverse --format='%H%x09%cI%x09%s' 'rust-v0.144.4^{}..rust-v0.145.0^{}' -- codex-rs/app-server codex-rs/app-server-protocol codex-rs/thread-store codex-rs/state codex-rs/rollout codex-rs/core/src/session
```
