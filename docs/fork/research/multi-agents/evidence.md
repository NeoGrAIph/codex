...
- Stable scope после обновления: `rust-v0.81.0`-`rust-v0.145.0`. Alpha tags не входят в основной timeline.
...
- Boundary `rust-v0.144.4^{}` -> `rust-v0.145.0^{}`: left/right `7/342`, merge-base `3380969a29134630d56feb6218e8e8dcc5e8196d`. Семь left-only commits относятся к patch-линии `0.144.x`; эквивалентные upstream изменения в `0.145.0` проверялись по patch/content, а не переносились по старым SHA.
...
- Включено в `EVID-144-guardian-reviewer`: `3eb56537eb598e157af8b415f6236c27b56b378f`, `0746e8a34574b4bf4721672c97fc6a94fd8bfad8`. Containment включает `rust-v0.144.0`; первый diff сузил Guardian prompt/tools только для `rust-v0.144.0`-`rust-v0.144.1` и был откатан в `rust-v0.144.2`, второй persist/restore effective reviewer и не был откатан.
...
### `EVID-144.2-guardian-rollback`

- Правосторонняя дельта: `rust-v0.144.1^{}..rust-v0.144.2^{}` содержит `2` коммита. Топология symmetric difference: left/right `1/2`; merge-base `77c42a202aa7e89189c1126021dcc054b1795e1d`. Left-only содержит предыдущий release anchor `44918ea10c0f99151c6710411b4322c2f5c96bea`.
- Release anchor: `a6645b6b8a656360fa16fb7e1c6721d0697d3d6a`, `2026-07-12T20:24:45-07:00`; он меняет только версию в `codex-rs/Cargo.toml` и не считается functional entry.
- Включённый commit: `32649bc5e6591ad8ea0b7b8ce073df447565ec7c`, `[release/0.144] Revert "Update auto review prompting" (#32672)`.
- Подтверждение: diff восстанавливает прежний Guardian policy/request layout и ordinary permissions developer message, удаляет Guardian-specific forced `ToolMode::Direct` и специальный ранний `add_tool_sources` с фиксированным набором `exec_command`/`write_stdin`/`view_image`. Derived Guardian config продолжает ограничивать skills, plugins, memories и MCP, но tool planning снова проходит через общий native path.
- Compatibility: rollback относится только к prompt/tool planning из `3eb56537eb598e157af8b415f6236c27b56b378f`; сохранение effective reviewer при resume из `0746e8a34574b4bf4721672c97fc6a94fd8bfad8` остаётся действующим.
- Автор commit зафиксировал `just fmt` и `just test -p codex-core guardian::tests` с 58 passed; локально в рамках docs-only актуализации эти tests не перезапускались.

### `EVID-144.3-advanced-reasoning`

- Правосторонняя дельта: `rust-v0.144.2^{}..rust-v0.144.3^{}` содержит `2` коммита. Топология symmetric difference: left/right `1/2`; merge-base `32649bc5e6591ad8ea0b7b8ce073df447565ec7c`. Left-only содержит предыдущий release anchor `a6645b6b8a656360fa16fb7e1c6721d0697d3d6a`.
- Release anchor: `78ad6e6bfd1d3b6a209acd3ef82172a96b25179c`, `2026-07-12T22:21:53-07:00`; он меняет только версию в `codex-rs/Cargo.toml`. Его release body описывает version-only выпуск без merged PR, однако правосторонняя Git-дельта фактически содержит отдельный direct commit.
- Включённый commit: `8a4d35a1e100efc2c64b72515668a84da663f067`, `feat(tui): add an advanced reasoning picker`.
- Подтверждение UI: `Max` и `Ultra` вынесены в explicit `More reasoning...` picker; повышение reasoning shortcut больше не входит в advanced efforts неявно; `Ultra` описан как demanding multi-agent режим с highest usage. Предупреждение при `Ultra` и configured concurrency `>= 8` сохранено в двухступенчатом flow.
- Подтверждение persistence: `EventMsg::ThreadSettingsApplied` обновляет state и live ThreadStore metadata; `ThreadMetadataPatch.reasoning_effort` получает clearable semantics, где omission означает no-op, а explicit `null` удаляет сохранённый effort. Cold resume восстанавливает persisted model/provider/effort только при отсутствии явного override, поэтому effort-derived MAv2 policy переживает restart/resume и не подменяется текущим implicit default.
- Scope: commit не меняет MAv2 handler/tool schema, agent control, mailbox, graph или lifecycle projections; Advanced Reasoning является UX/persistence path активации существующей core policy, а не вторым multi-agent runtime.
- Tests в upstream diff: `thread_resume_preserves_acknowledged_model_effort_and_approvals_reviewer_update`, `thread_settings_applied_updates_resume_metadata`, `thread_settings_applied_updates_live_metadata`, `thread_metadata_patch_round_trips_optional_clears`, `restored_conversation_ultra_remains_selected_after_switching_to_plan`, `advanced_reasoning_selection_in_plan_mode_uses_expected_scope`, `reasoning_up_shortcut_does_not_silently_enter_advanced_effort`, `advanced_only_reasoning_option_requires_explicit_selection` и новые picker snapshots.
- Containment: commit отсутствует в `rust-v0.144.2` и является ancestor `rust-v0.144.3`/`rust-v0.144.4`; `git describe --contains` возвращает `rust-v0.144.3~1`.

### `EVID-144.4-guardian-policy`

- Правосторонняя дельта: `rust-v0.144.3^{}..rust-v0.144.4^{}` содержит `2` коммита. Топология symmetric difference: left/right `1/2`; merge-base `8a4d35a1e100efc2c64b72515668a84da663f067`. Left-only содержит предыдущий release anchor `78ad6e6bfd1d3b6a209acd3ef82172a96b25179c`.
- Release anchor: `8c68d4c87dc54d38861f5114e920c3de2efa5876`, `2026-07-13T21:20:37-07:00`; он меняет только версию в `codex-rs/Cargo.toml` и не считается functional entry.
- Включённый commit: `d82b7e5d4c1c274bee0eb55f92ec12d017e78634`, `Use model catalog policies for Guardian auto review (#32875)`.
- Подтверждение: `ModelMessages.auto_review` и `AutoReviewMessages { policy }` добавляют model-catalog source для Guardian policy. Effective precedence: managed `guardian_policy_config`, затем policy выбранной Guardian model, затем встроенный `guardian_policy_prompt`; явная пустая catalog policy остаётся значением и не заменяется fallback.
- `ModelInfo::clear_instruction_messages` сохраняет `ModelMessages`, когда после удаления instruction templates в нём остаётся `auto_review`, поэтому base-instruction override не теряет catalog auto-review policy. Prewarmed Guardian session получает ту же effective policy.
- Scope: commit не возвращает узкий fixed tool set, удалённый в `rust-v0.144.2`, и не меняет MAv2 handler/tool schema, mailbox или agent lifecycle. Это adjacent Guardian review-subagent policy source.
- Tests: `guardian_review_session_config_prefers_managed_policy_over_catalog_policy`, `guardian_review_session_config_preserves_explicit_empty_catalog_policy`, `base_instruction_override_preserves_catalog_auto_review_messages`, `guardian_session_prewarms_and_is_reused_for_first_review`.

### Команды прохода `rust-v0.144.1` -> `rust-v0.144.4`

```bash
git rev-parse 'rust-v0.144.1^{}' 'rust-v0.144.2^{}' 'rust-v0.144.3^{}' 'rust-v0.144.4^{}'
git rev-list --left-right --count 'rust-v0.144.1^{}...rust-v0.144.2^{}'
git rev-list --left-right --count 'rust-v0.144.2^{}...rust-v0.144.3^{}'
git rev-list --left-right --count 'rust-v0.144.3^{}...rust-v0.144.4^{}'
git merge-base 'rust-v0.144.1^{}' 'rust-v0.144.2^{}'
git merge-base 'rust-v0.144.2^{}' 'rust-v0.144.3^{}'
git merge-base 'rust-v0.144.3^{}' 'rust-v0.144.4^{}'
git log --reverse --format='%H%x09%cI%x09%s' 'rust-v0.144.1^{}..rust-v0.144.4^{}'
git show --stat --name-only --oneline 32649bc5e6591ad8ea0b7b8ce073df447565ec7c 8a4d35a1e100efc2c64b72515668a84da663f067 d82b7e5d4c1c274bee0eb55f92ec12d017e78634
git merge-base --is-ancestor 8a4d35a1e100efc2c64b72515668a84da663f067 'rust-v0.144.2^{}'
git merge-base --is-ancestor 8a4d35a1e100efc2c64b72515668a84da663f067 'rust-v0.144.3^{}'
```
...
- `EVID-145-mav2-config-models-roles`: `ea15456284`, `92938d880e`, `03bb3b1236`, `9ff47868eb`, `21c37fb374`, `8a7c854bff`, `b7983c2a07`, `b00c9b2e16`, `687f05cb94`; `[agents]`/`AgentsToml`, feature stages, registry, models и role defaults.
- `EVID-145-mav2-resume-history-mailbox`: `088239294a`, `c28770a42f`, `2be648ba4a`; resume identity/lineage, history reconstruction и mailbox boundary.
- `EVID-145-mav2-control-and-tui`: `64c0e2fa1b`, `3f0669dbd6`, `d4982d0a33`, `e98765c212`; agent control tools, `list_agents`, app-server и TUI projections.
- `EVID-145-collaboration-world-state`: `4ebc61c0f8`, `4657ab06c6`, `20465d30ae`, `1fc15cc9a9`, `cf4c8cab7c`; proactive-mode instructions/settings и world-state section `collaboration_mode` с snapshot/diff rendering.
- `EVID-145-guardian-agent-runner`: `ea0fd84d94`, `60b9b551c1`, `0d78fa8846`, `6629e08702`, `83a4187837`, `68c6e5df2b`; Guardian narrow policy и shared `AgentRunner`. `60b9b551c1` является content-equivalent reapplication функциональности того же PR, а не Git patch-equivalent или доказательством ancestry с `d82b7e5d4c`.
...
git log --no-merges --reverse --format='%H%x09%cI%x09%s' 'rust-v0.144.4^{}..rust-v0.145.0^{}' -- codex-rs/config codex-rs/features codex-rs/core/src/agent codex-rs/core/src/guardian codex-rs/core/src/tools codex-rs/ext/agent codex-rs/app-server codex-rs/app-server-protocol codex-rs/tui codex-rs/protocol codex-rs/state
```
