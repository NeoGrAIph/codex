...
- Stable scope после обновления: `rust-v0.81.0`-`rust-v0.140.0`. Alpha tags не входят в основной timeline.
...
- Primary spark triage for `rust-v0.133.0`-`rust-v0.140.0` was attempted in release-sized batches but blocked by the `GPT-5.3-Codex-Spark` usage limit during this run; the recorded entries were locally verified with `git show`, `git show --stat`, `git merge-base --is-ancestor`, `git describe --contains` and release-note/changelog subjects instead.
