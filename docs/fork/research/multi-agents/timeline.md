...
Статус исследования: покрыты и зафиксированы stable-релизы `rust-v0.81.0`-`rust-v0.145.0`. Основной timeline строится только по stable tags. Alpha tags используются только как дополнительное evidence, если это потребуется отдельно.

## Семантика границ после `rust-v0.142.5`

Stable tags в этой части истории не образуют одну линейную цепочку, поэтому запись `A^{}..B^{}` ниже означает literal Git set difference: коммиты, достижимые из `B`, но не достижимые из `A`. Это правосторонняя дельта тега `B`, а не утверждение, что `A` является предком `B`. Топология проверена через `git rev-list --left-right --count A^{}...B^{}` и `git merge-base A^{} B^{}`:

- `rust-v0.142.5` -> `rust-v0.143.0`: left/right `6/258`, merge-base `27f22b54aef4d7e5eb6c564e969c961c74605461`. Шесть left-only коммитов остаются в ветке `0.142`: `aedb8f345a221b22105daad9fdf080988026fbca` и `3b8b60a58399ff79a41f8457710ae5a5fae5c39f` уже описаны в секции `rust-v0.142.2`-`rust-v0.142.5`; Bedrock catalog, auto-review revert, websocket backport и release anchor не переносятся повторно в `rust-v0.143.0`.
- `rust-v0.143.0` -> `rust-v0.144.0`: left/right `1/80`, merge-base `6afcf26d5d76c2f88b9096caa758931ffa673745`. Единственный left-only commit — release anchor `c4d748f586a84a3ed5b6aceb82e9a1db4abb1cda`; функциональные записи `rust-v0.144.0` берутся из правосторонней дельты.
- `rust-v0.144.0` -> `rust-v0.144.1`: left/right `1/4`, merge-base `3380969a29134630d56feb6218e8e8dcc5e8196d`. Единственный left-only commit — release anchor `767822446c7a594caa19609ca435281a9ec67e0d`; правосторонняя дельта `rust-v0.144.1` состоит из трёх installer/code-mode backports и нового release anchor.
- `rust-v0.144.1` -> `rust-v0.144.2`: left/right `1/2`, merge-base `77c42a202aa7e89189c1126021dcc054b1795e1d`. Left-only содержит предыдущий release anchor; right-only — Guardian rollback и новый release anchor.
- `rust-v0.144.2` -> `rust-v0.144.3`: left/right `1/2`, merge-base `32649bc5e6591ad8ea0b7b8ce073df447565ec7c`. Left-only содержит предыдущий release anchor; right-only — direct Advanced Reasoning commit и новый release anchor.
- `rust-v0.144.3` -> `rust-v0.144.4`: left/right `1/2`, merge-base `8a4d35a1e100efc2c64b72515668a84da663f067`. Left-only содержит предыдущий release anchor; right-only — Guardian model-catalog policy и новый release anchor.
- `rust-v0.144.4` -> `rust-v0.145.0`: left/right `7/342`, merge-base `3380969a29134630d56feb6218e8e8dcc5e8196d`. Семь left-only commits принадлежат patch-линии `0.144.x`; функциональные записи ниже получены из literal right-only inventory, а не из предположения о линейной ancestry.
...
- Коммит релиза: `c4d748f586a84a3ed5b6aceb82e9a1db4abb1cda`
- Дата: `2026-07-07T17:43:00-07:00`
- Сводка: release notes отдельно называют listing descendant threads; правосторонняя Git-дельта `rust-v0.142.5^{}..rust-v0.143.0^{}` также содержит изменения MAv2 namespace/guidance, durable persistence agent messages и world state, authoritative traversal agent graph, централизованные MAv2 delivery/telemetry, основу canonical lifecycle items и изменения Guardian review boundary.

### MAv2 namespace, разрешение делегирования и configurable mode hint (`#29067`, `#30274`, `#30493`)

- Коммиты: `49614a0391d83eec442ffeca1d4aa0fdeb119818` (`Namespace multi-agent v2 tools under collaboration (#29067)`), `79a8ffdbf7ca08820e0b44cab0261eb61ff72465` (`[codex] allow AGENTS.md and skills to authorize delegation (#30274)`), `da4c8ca57d40b074bdc1b5b1218851100150c56b` (`[codex] Add configurable multi-agent mode hint text (#30493)`).
- Для разработчиков: resolved default MAv2 namespace становится `collaboration`, а default usage hint и tests согласуются с `functions.collaboration.*`; TOML/schema override `tool_namespace` остаётся в `rust-v0.144.1`. Model-visible descriptions V1/MAv2 теперь признают explicit AGENTS.md или skill instructions разрешением на spawn. `features.multi_agent_v2.multi_agent_mode_hint_text` сохраняет effort-derived explicit/proactive behavior при отсутствии значения, а при наличии выбирает persisted `MultiAgentMode::Custom`; пустая строка подавляет built-ins. Legacy serialized `none` преобразуется в `Custom("")`.
- Почему: commit bodies и diffs связывают имена tools с фактической namespace surface, устраняют неоднозначность разрешения делегирования через AGENTS.md/skills и позволяют deployment заменить effort-derived mode instructions одной стабильной configured policy.
- Для пользователей: namespaced MAv2 calls по умолчанию используют `collaboration.*`; применимые workspace/skill instructions могут явно разрешить делегирование; оператор может заменить или подавить built-in policy text без смены reasoning effort.
- Совместимость и риски: `collaboration` нельзя описывать как безусловно фиксированный namespace, потому что explicit config override сохраняется. Custom hint text — model-visible policy и durable turn-context state, а не только UI copy.
- Доказательства: `EVID-143-namespace-policy`.

### Durable identity agent messages и persistence response items (`#29750`, `#29829`)

- Коммиты: `97dce078c57895d9cc184be923dc26b102e19493` (`chore: assign amsg_ IDs to agent messages (#29750)`), `b4f0f3eff1303ea445a218afb039e0079df0afce` (`Persist agent messages as response items (#29829)`).
- Для разработчиков: отсутствующие IDs `ResponseItem::AgentMessage` получают prefix `amsg_` и сохраняются через rollout/resume. Новые rollouts напрямую сохраняют уже подготовленный agent-message response item, оставляют только metadata `trigger_turn` для fork truncation и продолжают читать старые rollout items `InterAgentCommunication`.
- Почему: live Responses history и durable rollout history раньше по-разному представляли одно agent message и могли потерять live item ID после restart.
- Для пользователей: inter-agent messages сохраняют стабильную item identity через live app-server events, persistence и resume.
- Совместимость и риски: legacy rollout records остаются читаемыми; fork не должен удалять compatibility reader или выводить internal persistence metadata в model-visible envelope.
- Доказательства: `EVID-143-agent-message-persistence`.

### Authoritative agent graph и transitive listing descendants (`#29591`, `#29736`)

- Коммиты: `8057603d0c70930fe096302227f582b5b94496d8` (`feat(app-server): list descendant threads by ancestor (#29591)`), `ece1dfece07650458f84f3d2b35ffbde29ae48b7` (`[codex] Inject agent graph store into ThreadManager (#29736)`).
- Для разработчиков: optional `AgentGraphStore` внедряется в `ThreadManager`, а spawn, close, recursive resume и subtree archive/delete/feedback traversal проходят через него. Experimental `thread/list.ancestorThreadId` использует recursive traversal persisted `thread_spawn_edges`, возвращает strict descendants любой глубины, исключает ancestor и сохраняет immediate `parentThreadId` каждой строки. `parentThreadId` сохраняет значение direct-child filter; два filters взаимоисключающие.
- Почему: иначе clients вынуждены выполнять повторные race-prone direct-child requests, хотя runtime уже владеет persisted spawn graph для indexed traversal.
- Для пользователей: app-server clients могут одним paginated request восстановить всё spawned subtree и его иерархию из authoritative lineage.
- Совместимость и риски: response остаётся flat, filter — experimental. Исправленное traversal включает closed grandchildren под open edges; ancestry нельзя выводить из names или filesystem paths.
- Доказательства: `EVID-143-agent-graph-descendants`.

### Durable model world state и environment-sensitive AGENTS.md (`#29833`, `#29835`, `#29837`, `#29810`)

- Коммиты: `3e51b46eba036364a735f450c2bb4d7c35d48d2b` (`[1/3] core: make world state snapshots serializable (#29833)`), `fa036d39aadb160f819198d512efd1a2151a761b` (`[2/3] core: persist world state in rollouts (#29835)`), `a74771340db6eb81db39e81445a706346f14c139` (`[3/3] core: replay persisted world state (#29837)`), `f2f80ef442ff84612004bad064991283eb17cdb5` (`core: make AGENTS.md react to environment changes (#29810)`).
- Для разработчиков: каждая world-state section получает stable ID и serializable snapshot; full snapshots и RFC 7386 merge patches сохраняются как `RolloutItem::WorldState` и воспроизводятся через resume/fork/rollback/compaction, а approximate reconstruction из `TurnContextItem` удаляется. `AgentsMdManager` владеет environment-sensitive loading instructions и bounded replacement/removal updates через persisted section `agents_md`. Snapshots `EnvironmentsState` включают cwd, permissions-derived filesystem context и `<subagents>`.
- Почему: in-memory-only baselines могли повторить или подавить model-visible environment/subagent updates после resume, а deferred environment мог подключиться уже после однократного AGENTS.md discovery.
- Для пользователей: resumed/forked sessions получают environment, subagent и AGENTS.md updates относительно точного durable baseline; instructions нового environment появляются один раз и затем могут быть заменены или удалены.
- Совместимость и риски: старые binaries пропускают unknown world-state records и сохраняют остальные rollout lines; legacy/malformed history намеренно получает full render. Новый model-visible state должен быть bounded section со stable identity, а не parallel resume side channel.
- Доказательства: `EVID-143-world-state-agents-md`.

### Единый sink MAv2 communications и correlated lifecycle telemetry (`#30867`, `#30872`)

- Коммиты: `a98a21798c3301cfbeb6c323d6c0f6a804e08a57` (`Consolidate multi-agent v2 communication sends (#30867)`), `129ea2aaf5fb426d8ba683ee53f290742f41dd31` (`Log multi-agent communication lifecycle (#30872)`).
- Для разработчиков: direct messages, follow-up tasks, completion results и initial messages MAv2 spawn проходят через `AgentControl::submit_inter_agent_communication`, который централизует capacity/submission и last-task bookkeeping. Events `codex_otel.agent_communication` фиксируют send/receive для spawn/message/followup/result с kind, sender/receiver IDs, content и correlated submission ID; receive эмитится после mailbox enqueue.
- Почему: разделённые outbound paths не позволяли получить полную lifecycle observability и давали новым paths обойти common bookkeeping.
- Для пользователей: tool shape и message ordering не меняются; оператор может связать каждую MAv2 communication от успешного send до mailbox receipt.
- Совместимость и риски: telemetry — слой observability, а не runtime authority. Сохраняются submission IDs, capacity checks, mailbox ordering, protocol types, rollout data и model-visible context.
- Доказательства: `EVID-143-mav2-communication`.

### Основа canonical items agent lifecycle и legacy fanout (`#30282`, `#31296`)

- Коммиты: `a107b84967eb9a3444fd2d4de03f200337acd52b` (`feat(protocol): define missing rollout turn items (#30282)`), `b9b934e99b57aa1908a7cd83adee9faf6d0b8f7d` (`refactor(protocol): map canonical tool items to legacy events (#31296)`).
- Для разработчиков: определяются canonical core `TurnItem::CollabAgentToolCall` и `TurnItem::SubAgentActivity` со stable IDs и complete state; app-server получает conversion/history handling `ThreadItem`, а centralized canonical-to-legacy mappings `EventMsg` сохраняют rollout-trace compatibility.
- Почему: begin/end legacy events частичные и требуют full-history replay для reconstruction materialized item; canonical snapshots готовят suffix-only paginated projection, сохраняя работу существующих raw-event consumers.
- Для пользователей: producer behavior на этом шаге не меняется; создаются stable item identity и compatibility projection для последующей миграции `rust-v0.144.0`.
- Совместимость и риски: canonical `TurnItem` становится целевым источником истины, но legacy event fanout сохраняется до миграции consumers. Второй fork-specific mapping layer не нужен.
- Доказательства: `EVID-143-canonical-item-foundation`.

### Trust boundary Guardian approvals и ownership shell tools (`#27045`, `#31267`)

- Коммиты: `50eee505a3bbd7a66a73360ae28ea4968f79dd36` (`feat(guardian): include connected account email in app reviews (#27045)`), `aa94ea139744f0a9dd76343805cc96649d9f2658` (`chore(approvals) consolidate guardian calls for shell tools (#31267)`).
- Для разработчиков: trusted built-in metadata `codex_apps` может добавить trimmed connected-account email в private Guardian app-review action; custom MCP metadata не может его spoof, email не попадает в model-visible specs, app-server approval events или analytics. Shell/apply-patch runtimes перестают напрямую вызывать Guardian-specific logic и передают общий `ApprovalAction` через tool orchestrator.
- Почему: auto review нужен authenticated account identity для различения work/personal destinations, а tool runtimes должны зависеть от approval abstraction, а не владеть Guardian routing.
- Для пользователей: auto review точнее оценивает identity destination connector; shell/apply-patch approval продолжает работать через centralized path.
- Совместимость и риски: connected-account email — sensitive review-only metadata. Нужно сохранить проверку trusted built-in source и не расширять projections.
- Доказательства: `EVID-143-guardian-approvals`.

## `rust-v0.144.0`

- Коммит релиза: `767822446c7a594caa19609ca435281a9ec67e0d`
- Дата: `2026-07-09T08:59:13-07:00`
- Сводка: release notes отдельно называют Ultra/high-concurrency warning; правосторонняя Git-дельта `rust-v0.143.0^{}..rust-v0.144.0^{}` также завершает production canonical V1/V2 agent lifecycle, сохраняет canonical turn items для paginated rollouts и усиливает Guardian review prompt/tool и resume behavior.

### Canonical V1/V2 agent activity и lifecycle wait (`#31299`, `#31300`, `#31301`)

- Коммиты: `1bd9d841ca9ec8c99e4843f5be3d1d71cb5d8169` (`feat(core): emit canonical sub-agent activity items (#31299)`), `058d97c5dcf572ef7084e488bbb108d24aa19e54` (`feat(core): emit canonical collab tool call items (#31300)`), `6b4882528eaf2d927904bba37c48e40d925c35d1` (`feat(core): emit canonical collab wait items (#31301)`).
- Для разработчиков: MAv2 spawn/message/follow-up/interrupt эмитит completed `TurnItem::SubAgentActivity`; V1 spawn/send/resume/close — canonical `TurnItem::CollabAgentToolCall`; V1 и V2 wait — canonical collab lifecycle. App-server использует completed canonical items для watcher cleanup и игнорирует duplicate mapped legacy events. V1 wait сохраняет target/status snapshots; V2 mailbox wait по-прежнему оставляет их пустыми.
- Почему: canonical item lifecycle — стабильный runtime/read-model source, а legacy begin/end events остаются compatibility projection, а не producer authority.
- Для пользователей: app-server clients сохраняют прежнюю visible activity/tool semantics со stable canonical item IDs и complete final state.
- Совместимость и риски: semantics V1 и V2 wait намеренно различаются. Вместе обновляются canonical producers, legacy mapping, app-server side effects и tests.
- Доказательства: `EVID-144-canonical-agent-items`.

### Persistence canonical turn items в paginated rollouts (`#30188`)

- Коммиты: `2342b2c2a66b5ac2a07278afb4f5f38700be452e` (`feat(rollout): persist TurnItems for paginated thread rollouts (#30188)`).
- Для разработчиков: threads с `history_mode = "paginated"` сохраняют `ItemCompleted` с complete canonical snapshots `TurnItem` и опускают redundant legacy projections; legacy threads сохраняют прежнюю event policy. Forks наследуют source history mode, локально созданные paginated items получают stable IDs, а app-server/thread-store/state readers понимают canonical form.
- Почему: paginated projection нужны stable turn/item IDs и suffix-reducible complete snapshots вместо replay всего rollout для correlation begin/end fragments.
- Для пользователей: paginated thread history может инкрементально материализовать collab и sub-agent activity без изменения legacy-thread behavior.
- Совместимость и риски: rollout имеет один immutable history mode; смешивать или silently convert legacy/paginated records нельзя. Agent lifecycle обязан работать с обеими persistence policies.
- Доказательства: `EVID-144-paginated-turn-items`.

### TUI warning для Ultra при высокой concurrency (`#31621`)

- Коммиты: `927004c06dc55565af17f0bc8eeb5e35fb990351` (`tui: warn on Ultra with high multi-agent concurrency (#31621)`).
- Для разработчиков: после explicit выбора Ultra TUI эмитит warning при `features.multi_agent_v2.max_concurrent_threads_per_session >= 8`, показывая configured thread count, вычисленный maximum subagent count и управляющую setting. Проверка общая для model/reasoning pickers, Plan-mode scope selection и reasoning shortcuts.
- Почему: Ultra может proactively delegate, а высокая configured concurrency быстро увеличивает usage; release note фиксирует это как explicit user-facing behavior.
- Для пользователей: concrete concurrency/usage tradeoff показывается сразу при выборе Ultra; lower limits, другие efforts и startup behavior не меняются.
- Совместимость и риски: это только warning projection. Config/runtime остаётся источником истины concurrency, mechanism activation не меняет threshold.
- Доказательства: `EVID-144-ultra-concurrency-warning`.

### Временное сужение tools Guardian reviewer и continuity при resume (`#31480`, `#30278`)

- Коммиты: `3eb56537eb598e157af8b415f6236c27b56b378f` (`Update auto review prompting (#31480)`), `0746e8a34574b4bf4721672c97fc6a94fd8bfad8` (`[codex] Preserve reviewer when resuming threads (#30278)`).
- Для разработчиков: Guardian auto-review instructions упрощаются, ordinary permissions developer message удаляется, reviewer tools ограничиваются exec/view-image через native spec-plan path. Эта prompt/tool конфигурация действует в `rust-v0.144.0`-`rust-v0.144.1` и откатывается в `rust-v0.144.2`. Effective reviewer сохраняется с каждым turn и восстанавливается при resume, если resume request не задаёт explicit override; эта часть не откатывается.
- Почему: неоднозначные sandbox instructions и нерелевантные tools ухудшали auto-review performance, а config при restart мог silently заменить существующего reviewer thread.
- Для пользователей: в этих двух релизах auto review получает более узкий purpose-built context/tool set; continuity reviewer после resume сохраняется и в последующих patch-релизах.
- Совместимость и риски: fixed narrow Guardian tool set нельзя считать current architecture после `rust-v0.144.2`. Reviewer остаётся durable turn/session state: current config используется только как fallback при отсутствии persisted/explicit значения. Guardian остаётся отдельным review subagent path, а не MAv2 role.
- Доказательства: `EVID-144-guardian-reviewer`.

## `rust-v0.144.1`

- Коммит релиза: `44918ea10c0f99151c6710411b4322c2f5c96bea`
- Дата: `2026-07-09T15:10:27-07:00`
- Сводка: patch-релиз надёжности installer и code mode.
- Результат triage: в правосторонней дельте `rust-v0.144.0^{}..rust-v0.144.1^{}` прямых multi-agent/collab/subagent/Guardian изменений нет. Три functional backport-коммита относятся к parsing release metadata, Darwin code-mode installation и embedded V8 fallback.
- Evidence: `EVID-144.1-no-change`.

## `rust-v0.144.2`

- Коммит релиза: `a6645b6b8a656360fa16fb7e1c6721d0697d3d6a`
- Дата: `2026-07-12T20:24:45-07:00`
- Topology boundary от `rust-v0.144.1`: left/right `1/2`, merge-base `77c42a202aa7e89189c1126021dcc054b1795e1d`; left-only содержит предыдущий release anchor.

### Rollback Guardian prompting и tool planning (`#32672`)

- Коммит: `32649bc5e6591ad8ea0b7b8ce073df447565ec7c` (`[release/0.144] Revert "Update auto review prompting" (#32672)`).
- Для разработчиков: восстановлены прежние Guardian policy/request layout и ordinary permissions developer message; удалены Guardian-specific forced `ToolMode::Direct` и ранняя подстановка фиксированного набора exec/write/view-image tools. Guardian снова использует общий native tool-planning path поверх своей ограниченной derived config.
- Для пользователей: auto review возвращается к прежнему prompt/tool behavior; сохранённый reviewer по-прежнему переживает resume.
- Совместимость и риски: откатан только `3eb56537eb598e157af8b415f6236c27b56b378f`; persistence reviewer из `0746e8a34574b4bf4721672c97fc6a94fd8bfad8` не затронут. Fixed narrow tool set больше не является current invariant.
- Evidence: `EVID-144.2-guardian-rollback`.

## `rust-v0.144.3`

- Коммит релиза: `78ad6e6bfd1d3b6a209acd3ef82172a96b25179c`
- Дата: `2026-07-12T22:21:53-07:00`
- Topology boundary от `rust-v0.144.2`: left/right `1/2`, merge-base `32649bc5e6591ad8ea0b7b8ce073df447565ec7c`; release body называет выпуск version-only, но правосторонняя дельта содержит отдельный direct commit `8a4d35a1e`.

### Explicit Advanced Reasoning и durable effort

- Коммит: `8a4d35a1e100efc2c64b72515668a84da663f067` (`feat(tui): add an advanced reasoning picker`).
- Для разработчиков: `Max`/`Ultra` вынесены в explicit advanced picker, shortcut больше не входит в них неявно. `ThreadSettingsApplied` становится producer persisted/live metadata, `reasoning_effort` получает clearable patch semantics, а cold resume восстанавливает model/provider/effort при отсутствии явного override.
- Для пользователей: advanced efforts выбираются осознанно; выбранный `Ultra` и связанная effort-derived MAv2 policy переживают restart/resume, а explicit unset effort не заменяется глобальным default.
- Совместимость и риски: MAv2 tools, handlers, mailbox и lifecycle не меняются. UI обязан отличать implicit defaults от explicit session/profile overrides, иначе resume перетрёт thread-owned settings.
- Evidence: `EVID-144.3-advanced-reasoning`.

## `rust-v0.144.4`

- Коммит релиза: `8c68d4c87dc54d38861f5114e920c3de2efa5876`
- Дата: `2026-07-13T21:20:37-07:00`
- Topology boundary от `rust-v0.144.3`: left/right `1/2`, merge-base `8a4d35a1e100efc2c64b72515668a84da663f067`; left-only содержит предыдущий release anchor.

### Guardian policy из model catalog (`#32875`)

- Коммит: `d82b7e5d4c1c274bee0eb55f92ec12d017e78634` (`Use model catalog policies for Guardian auto review (#32875)`).
- Для разработчиков: `ModelMessages.auto_review.policy` становится model-catalog source для Guardian policy. Precedence: managed `guardian_policy_config`, затем catalog policy выбранной Guardian model, затем встроенный prompt; explicit empty catalog policy не подменяется fallback. Очистка instruction messages сохраняет catalog `auto_review` metadata.
- Для пользователей: managed deployment сохраняет высший приоритет, а выбранная Guardian model может поставлять собственную auto-review policy без fork-specific prompt wiring.
- Совместимость и риски: изменение не возвращает fixed narrow tool set и не меняет MAv2 runtime. Model-catalog policy — adjacent Guardian review-subagent configuration, её нельзя смешивать с ordinary model instructions или MAv2 role metadata.
- Evidence: `EVID-144.4-guardian-policy`.

## `rust-v0.145.0`

- Release commit: `25af12f7e61572b0bc18ddb1008be543b91519b0`.

### MAv2 config, models и roles

- Включённые commits: `ea15456284`, `92938d880e`, `03bb3b1236`, `9ff47868eb`, `21c37fb374`, `8a7c854bff`, `b7983c2a07`, `b00c9b2e16`, `687f05cb94`.
- `[agents]`/`AgentsToml`, staged feature gates, agent-model registry и role defaults заменяют прежний CSV/job-shaped baseline; effective role/model selection остаётся native runtime contract.
- Evidence: `EVID-145-mav2-config-models-roles`.

### Resume identity, history и mailbox

- Включённые commits: `088239294a`, `c28770a42f`, `2be648ba4a`.
- Resume сохраняет agent/thread identity и lineage, а mailbox state и delivery остаются отдельной runtime границей вместо восстановления через неявный prompt state.
- Evidence: `EVID-145-mav2-resume-history-mailbox`.

### Control surface и TUI

- Включённые commits: `64c0e2fa1b`, `3f0669dbd6`, `d4982d0a33`, `e98765c212`.
- `list_agents` больше не возвращает task message, parent-owned child остаётся read-only для direct input, picker сохраняет liveness, а TUI navigation выбирает path-backed agents.
- Evidence: `EVID-145-mav2-control-and-tui`.

### Collaboration world state

- Включённые commits: `4ebc61c0f8`, `4657ab06c6`, `20465d30ae`, `1fc15cc9a9`, `cf4c8cab7c`.
- Proactive-mode instructions/settings получают отдельную world-state section `collaboration_mode` с snapshot по `ModeKind` и diff rendering; пустое mode message подавляется.
- Evidence: `EVID-145-collaboration-world-state`.

### Guardian и AgentRunner

- Включённые commits: `ea0fd84d94`, `60b9b551c1`, `0d78fa8846`, `6629e08702`, `83a4187837`, `68c6e5df2b`.
- Guardian остаётся узким review agent с отдельной tool policy, а `AgentRunner` задаёт общий execution boundary; `60b9b551c1` является content-equivalent reapplication той же функциональности, а не Git patch-equivalent или переносом старого commit SHA.
- Evidence: `EVID-145-guardian-agent-runner`.
