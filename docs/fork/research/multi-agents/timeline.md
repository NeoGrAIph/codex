...
Статус исследования: покрыты и зафиксированы stable-релизы `rust-v0.81.0`-`rust-v0.144.1`. Основной timeline строится только по stable tags. Alpha tags используются только как дополнительное evidence, если это потребуется отдельно.
...
### Изоляция tools Guardian reviewer и continuity при resume (`#31480`, `#30278`)
...
- Для разработчиков: Guardian auto-review instructions упрощаются, ordinary permissions developer message удаляется, reviewer tools ограничиваются exec/view-image через native spec-plan path. Effective reviewer сохраняется с каждым turn и восстанавливается при resume, если resume request не задаёт explicit override.
...
- Для пользователей: auto review получает более узкий purpose-built context/tool set, а resumed threads продолжают использовать прежнего reviewer без explicit изменения.
- Совместимость и риски: reviewer — durable turn/session state; current config используется только как fallback при отсутствии persisted/explicit значения. Guardian остаётся отдельным review subagent path, а не MAv2 role.
