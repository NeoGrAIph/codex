...
Статус исследования: покрыты stable-релизы от первого внешнего MCP server commit `147a940449839b116b220b7e7d016d2a2890c134` до `rust-v0.145.0`. Основной timeline строится по stable-contained commits; alpha/side-branch commits не включены в основной поток без отдельного указания.
...
## `rust-v0.144.2`-`rust-v0.144.4`

- Release commits: `a6645b6b8a656360fa16fb7e1c6721d0697d3d6a` (`rust-v0.144.2`, `2026-07-12T20:24:45-07:00`), `78ad6e6bfd1d3b6a209acd3ef82172a96b25179c` (`rust-v0.144.3`, `2026-07-12T22:21:53-07:00`), `8c68d4c87dc54d38861f5114e920c3de2efa5876` (`rust-v0.144.4`, `2026-07-13T21:20:37-07:00`).
- Существенных изменений external MCP нет. Три functional commits относятся к rollback Guardian prompting/tool planning, Advanced Reasoning и thread resume metadata, затем model-catalog Guardian policy; targeted diff MCP config/client/runtime/OAuth/tool/resource/app-server/TUI paths пуст.
- Guardian rollback — проверенный near-miss: возврат к общему tool planning не включает MCP в Guardian, потому что derived Guardian config продолжает очищать `mcp_servers` и отключать Apps/Plugins/Memories.
- Совместимость/риски: общие tool-planning изменения нельзя автоматически классифицировать как MCP exposure delta; источником истины остаются effective MCP registrations/runtime snapshot и фактически построенный router.
- Доказательства: `EVID-144.2-144.4-no-mcp-delta`.

## `rust-v0.145.0`

- Release commit: `25af12f7e61572b0bc18ddb1008be543b91519b0`.
- Topology boundary от `rust-v0.144.4`: left/right `7/342`, merge-base `3380969a29134630d56feb6218e8e8dcc5e8196d`; поэтому `rust-v0.144.4^{}..rust-v0.145.0^{}` используется как literal right-only inventory, а не как ancestry claim.

### Thread-owned runtime и cache coherence

- Включённые commits: `2f7d89b141`, `1447cee36b`, `42c5d3c80d`, `3307ea8b63`, `1bbdb32789`, `79177c3e20`, `f24e695470`, `19940967bd`.
- Live `McpRuntime` становится thread-owned, `McpRuntimeSnapshot` остаётся request/step-pinned, а `McpToolCatalogCache` — process-scoped; availability-aware reuse и единый snapshot на sampling request удерживают routing и model-visible catalog в согласованном lifecycle.
- Evidence: `EVID-145-runtime-ownership-cache`.

### Startup, transport и OAuth

- Включённые commits: `6ced1ac5eb`, `cbdee7976b`, `6962a2ecae`, `8b2c84ddcc`, `2e156cbe31`, `44954d1b4b`.
- Remote stdio получает bounded lines и serialized stdin writes; OAuth credential store pinning и refresh сериализуются, startup timeout применяется при создании client, а optional OAuth discovery больше не блокирует thread startup.
- Evidence: `EVID-145-startup-transport-oauth`.

### Hosted Apps, files и routing

- Включённые commits: `23a09eb3c3`, `b58952b0fa`, `0e19d5e908`, `8347b8de21`, `8604689ec5`, `2b486b4676`, `6bf4845b60`.
- Apps получают codex SKU/originator, schema-aware file payloads, HTTP-client routing и injectable tool cache; OpenAI docs requests атрибутируются Codex, а Apps MCP endpoint переносится в plugin service.
- Evidence: `EVID-145-hosted-apps-files-routing`.

### Content, wire и approval projection

- Включённые commits: `800715d201`, `cbc83d961e`, `643de86a19`, `e52c35b000`.
- Из MCP tool-call metadata удаляются template IDs, encrypted content сохраняется в outputs, dynamic tools/code mode получают audio outputs, а approval rejection reason передаётся вызывающей стороне.
- Evidence: `EVID-145-content-wire-approval`.
