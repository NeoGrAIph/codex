...
Статус документа: нормализованная карта нативных source-of-truth и extension points для Codex как MCP client/runtime на stable `rust-v0.145.0` (`25af12f7e61572b0bc18ddb1008be543b91519b0`). Историческая хронология покрыта до `rust-v0.145.0` в `timeline.md`; проверочные команды и доказательства по коммитам приведены в `evidence.md`.
...
- Владение runtime/client: `codex-rs/codex-mcp/src/mcp/mod.rs` владеет `McpConfig`, сборкой effective servers, снимками auth status, чтением resources и Apps MCP helpers; `codex-rs/codex-mcp/src/connection_manager.rs` владеет запущенными clients, startup status, агрегацией tools/resources/templates, маршрутизацией вызовов и elicitation requests. `codex-rs/codex-mcp/src/runtime.rs` владеет live `McpRuntime`: атомарно заменяемым connection set, context разрешения environment/sandbox и teardown; `codex-rs/core/src/mcp.rs` владеет thread projection и process-wide `McpToolCatalogCache`. `codex-rs/core/src/session/mcp_runtime.rs` сохраняет step-pinned `McpRuntimeSnapshot`: точные config, manager и runtime context одного model step, но больше не является единственным владельцем live connection lifecycle.
- Кэш tool catalog: `codex-rs/codex-mcp/src/tool_catalog_cache.rs` владеет process-wide `McpToolCatalogCache`. Пока startup не завершён, cached tools могут быть показаны модели напрямую; cache очищает connection-specific instructions/annotations, а вызов всё равно разрешается через live connection и отсутствующий live tool не выполняется. Server capability может отключить reuse; cache ускоряет startup между sessions, но не является live runtime authority.
- Реализация транспорта: `codex-rs/rmcp-client/src/rmcp_client.rs`, `stdio_server_launcher.rs`, `executor_process_transport.rs`, `http_client_adapter.rs`, `oauth.rs`, `oauth/store_lock.rs`, `oauth/resolved_store.rs`, `oauth/refresh_lock.rs`, `oauth/refresh_transaction.rs`, `auth_status.rs` и `perform_oauth_login.rs` владеют RMCP stdio/HTTP transport, protected-resource OAuth discovery, хранением OAuth, lifecycle pinning credential store, межпроцессной сериализацией refresh, authoritative read-refresh-write transaction и очисткой processes. HTTP MCP и OAuth traffic выбранного executor plugin используют HTTP client владеющего executor, а не host-local bootstrap path.
- Видимая модели экспозиция tools: `codex-rs/core/src/session/step_context.rs`, `codex-rs/core/src/mcp_tool_exposure.rs`, `codex-rs/core/src/tools/registry.rs`, `codex-rs/core/src/tools/handlers/mcp.rs`, `codex-rs/core/src/mcp_tool_call.rs`, `codex-rs/core/src/mcp_openai_file.rs`, `codex-rs/tools/src/mcp_tool.rs` и `codex-rs/tools/src/tool_config.rs` владеют MCP tool snapshot на один sampling request, общим runtime contract `CoreToolRuntime`, отложенной через tool search экспозицией по умолчанию, прямым fallback, namespace, environment filesystem, hook payloads, approval review и tool-call execution. MCP handlers собираются до router planning, а не подключаются как побочный dispatch path.
...
4. `McpConnectionManager` запускает включённые servers с startup timeout, публикует per-server status и хранит metadata. Process-wide tool cache может дать модели ранний catalog до завершения startup; connection-specific metadata из него удалена, а последующий tool call требует соответствующий live tool.
5. Thread-owned `McpRuntime` атомарно публикует connection set и целиком завершает его при teardown. App-server `config/mcpServer/reload` ставит strict refresh в очередь; старые step snapshots остаются валидными для in-flight work, но не владеют новым thread lifecycle.
...
3. App-server `mcpServer/resource/read` supports thread-bound reads through loaded thread runtime, or threadless reads from latest config with config-level environment fallback. Threadless manager может работать без event channel: notifications пропускаются, interactive elicitations управляемо отклоняются.
...
6. Host-owned `codex_apps` подключается через plugin-service endpoint; это замена transport endpoint, а не разрешение обходить catalog, auth identity и connector policy.
