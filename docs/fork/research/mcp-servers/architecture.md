...
Статус документа: текущая карта HEAD по source-of-truth и extension points для Codex как MCP client/runtime. Исторический timeline покрыт до `rust-v0.142.0` в `timeline.md`; проверочные команды и commit evidence - в `evidence.md`.
...
2. `McpRuntimeContext` resolves each server's environment. Local stdio requires a local environment; remote stdio requires explicit absolute `cwd`; HTTP can run without stdio environment fallback. Sandbox metadata is scoped to the resolved server environment.
