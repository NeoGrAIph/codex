...
- keyed lifecycle guards сериализуют exact resume/close по identity; root guard охватывает полный Open-tree traversal, а graph-derived parent имеет приоритет над stale rollout metadata;
...
- Concurrent resume/close линеаризуется целиком: невозможен successful close с возвращённым stale root или частично вновь загруженным V1 subtree.
