...
| --- | --- | --- |
| V2/V1 namespace coexistence, schema separation, role/model projection | `just test -p codex-core spec_plan`; `just test -p codex-core multi_agents_spec` | 28 + 11 passed |
| Default native `collaboration.spawn_agent` сохраняет reserved schema, а projected V1 показывает role catalog | `just test -p codex-core v2_first_request_keeps_reserved_spawn_agent_canonical_while_v1_projects_roles`; `just test -p codex-core responses_lite_exposes_native_v2_and_projected_v1_namespaces` | runs `07db8683-ae71-4bc7-9833-93cbf4d358c8`, `86c5735a-9286-466d-986f-0f53930acb53`, passed; Responses Lite проверен для встроенных `gpt-5.6-sol` и `gpt-5.6-terra`, весь передаваемый provider tool object сравнивается с независимым literal fixture |
| Project-local roles имеют приоритет в model-visible catalog | `just test -p codex-core mixed_legacy_and_standalone_agent_role_sources_merge_with_precedence`; `just test -p codex-core spawn_tool_spec` | runs `e0059cd8-cc1b-4e7a-add3-a38e08390dec`, `cb116741-1a8b-4e17-a9f6-1f8c606cf3fa`, passed; loader test включает project/global metadata-only roles без `config_file` |
...
| `just test -p codex-core` | `d440ffda-4e81-46c0-a0de-2271a3c63dc4` | 2957 passed, 24 failed, 15 skipped, exit 100 | Failures относятся к ambient project config/trust context, git/realtime workspace discovery, incompatible bundled zsh/GLIBC, sandbox/helper/request-permissions/guardian host restrictions либо unrelated timing. Multi-agent/lifecycle feature failures отсутствуют; candidate-adjacent `spawned_subagent_execpolicy_amendment_propagates_to_parent_session` прошёл отдельно в run `dddc8ed7-583b-4d11-9ff0-0e30d7085353`. |
...
| полный `just test` | `c3ee8adc-be75-40f7-a56f-79a3588f9176` | 11997 passed, 65 failed, 28 skipped, exit 100 | Все failures относятся к ambient/upstream категориям: bundled zsh/GLIBC, project discovery/trust context, unrelated TUI snapshots, sandbox/helper/request-permission/guardian timing, MCP/network baseline и другие unrelated tests. Multi-agent/lifecycle feature failures отсутствуют; feature-focused tests в полном прогоне прошли. |
...
Post-test gates выполнены в требуемом порядке: после удаления generated snapshots текущей проверки `snap_new_count=0`; для текущего reserved-schema correction `just fix -p codex-core` и финальный `just fmt` завершились с exit 0. После последнего `fix`/`fmt` tests по repo policy не перезапускались. `git diff --check` прошёл.
