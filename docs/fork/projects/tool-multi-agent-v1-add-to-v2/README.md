...
- Default native `collaboration.spawn_agent` остаётся provider-reserved/canonical и не получает dynamic role/model/usage metadata; projected V1 role catalog доступен модели, ставит materialized project-local roles перед user/global roles, ограничен 32 entries, 768 JSON-bytes на entry и 8192 JSON-bytes суммарно и объединяет configured roles с отдельным built-in catalog без повторного чтения файлов; native V1, projected V1 и V2 runtime применяют тот же immutable per-Config role snapshot.
