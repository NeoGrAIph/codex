...
`multi_agents_spec.rs` остаётся владельцем обеих schemas. Default native `collaboration.spawn_agent` сохраняет provider-reserved schema и не получает dynamic role/model/usage metadata, потому что строгие модели проверяют это имя против configured schema. Projected V1 использует `SpawnAgentModelCatalogDisplay::OmitForProjection`: explicit model override поддерживается, но список моделей не копируется из native V2 description. `agent_type` сохраняет role discovery, потому что строковое имя роли нельзя корректно выбрать без model-visible catalog. Если V2 tools явно перенесены в non-reserved configured namespace, native V2 schema может показывать role/model metadata по `hide_spawn_agent_metadata`, потому что это уже не reserved `collaboration.spawn_agent`.

Role catalog строится в `agent/role/spawn_tool_spec.rs`. Project/user declarations поступают только из уже materialized effective `Config.agent_roles`, built-ins — из `built_in::configs()`; planning не перечитывает project role files и поэтому не получает TOCTOU между turn-ами. Loader отдельно сохраняет model-visible порядок эффективных ролей по убыванию приоритета config layer: project-local роли идут первыми и при совпадении имени уже перетёрли user/global declaration, затем следуют нижележащие configured roles и built-ins. Materialization добавляет role-locked model/reasoning/service-tier guidance. Projected V1 и non-reserved native V2 catalogs ограничены 32 entries, 768 JSON-encoded bytes на entry и 8192 JSON-encoded bytes суммарно, всегда сохраняют `default` и явно сообщают об усечении/пропущенных roles. V1-only сохраняет прежнюю форму description builder, но получает configured role descriptions и settings из того же immutable Config snapshot. Trusted loader ограничивает один role file 65536 bytes, читает его bounded stream-ом, ограничивает discovery 256 files и `developer_instructions` 32768 bytes.
...
- Existing V1-only schemas сохраняются, а default native V2 `collaboration.spawn_agent` сохраняет provider-reserved canonical schema независимо от projected V1 role catalog. Native V1 lifecycle остаётся best-effort и не получает transactional V2 latch/coordinator/termination timeout; native V2 и projected V1-under-V2 используют один transactional path. Общие intentional hardening-изменения: fail-fast защита dynamic namespace от фактически активного built-in surface и immutable bounded snapshot configured role files вместо повторного чтения файла существующей session.
