...
The `feature/140/workbench-read-model` slice introduces a first-class ephemeral read model for the normal `/agent` workbench rows. Its source is still `AgentNavigationState.ordered_threads()` plus already-bounded per-thread summaries from `ThreadEventStore`; it owns the derived summary line, row label, row description, selected detail, current-row marker and search text. It does not own action execution, app-server requests, persistence, hidden-row state or lifecycle authority. This gives future `dismiss`, `retry` and `stop all` slices one read projection to extend without creating a parallel workbench registry.
